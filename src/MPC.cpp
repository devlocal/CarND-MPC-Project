...
// Set to true to use yaw rate in state prediction
const bool kCtrvModel = false;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // any anything that may be beneficial.
    for (int t = 0; t < N; ++t) {
      const AD<double> &v = vars[v_start + t];
      const AD<double> &cte = vars[cte_start + t];
      ...
      if (t > 0) {
        ...
        // Penalize cte increase
        fg[0] += (CppAD::pow(cte, 2) - CppAD::pow(cte0, 2)) * lambda_dcte;
      }

      if (t < N - 1) {
        const AD<double> &delta = vars[delta_start + t];

        // Penalize high-speed turns
        fg[0] += v * CppAD::pow(delta, 2) * lambda_v_delta / (1 + t * 0.2);
      }
    }
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> yaw_rate = (v0 / Lf) * delta0;
      AD<double> future_psi = psi0 + yaw_rate * dt;
      if (kCtrvModel && abs(yaw_rate) > 1e-16) {
        AD<double> v_yr = v0 / yaw_rate;
        fg[1 + x_start + t] = x1 - (x0 + v_yr * (CppAD::sin(future_psi) - CppAD::sin(psi0)));
        fg[1 + y_start + t] = y1 - (y0 + v_yr * (CppAD::cos(psi0) - CppAD::cos(future_psi)));
      } else {
        fg[1 + x_start + t] = x1 - (x0 + v0 * CppAD::cos(psi0) * dt);
        fg[1 + y_start + t] = y1 - (y0 + v0 * CppAD::sin(psi0) * dt);
      }
      fg[1 + psi_start + t] = psi1 - future_psi;
      fg[1 + v_start + t] = v1 - (v0 + a0 * dt);
      fg[1 + cte_start + t] = cte1 - (f0 - y0 + v0 * CppAD::sin(epsi0) * dt);
      fg[1 + epsi_start + t] = epsi1 - (future_psi - psides0);
    }
  }
};
...
vector<double> MPC::Solve(const Eigen::VectorXd &state, const Eigen::VectorXd &coeffs) {
  ...
  for (size_t t = 0; t < N; ++t) {
    xpts_[t] = solution.x[x_start + t];
    ypts_[t] = solution.x[y_start + t];
  }
